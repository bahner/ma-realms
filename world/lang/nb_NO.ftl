world.help.commands = kommandoer: help | list | show [did] | describe [did] | lang [show|set <cid>|clear]
world.list.empty = @world objekter: (ingen)
world.private.status.on = @world private=on (nye brukere må banke på)
world.private.status.off = @world private=off (åpen inngang)
world.owner.required = @world bare world-eier kan kjøre den kommandoen.
world.private.on = privat modus aktivert; nye brukere må banke på
world.private.off = privat modus deaktivert; inngang er nå åpen
world.private.usage = @world bruk: @world.private [on|off|status]
world.knock.empty = knock-innboksen er tom
world.lang.show = lang_cid={$cid}
world.lang.cleared = lang_cid tømt (lagre verden for å persistere)
world.lang.set = lang_cid satt til {$cid} (lagre verden for å persistere)
world.lang.usage.set = @world bruk: @world.lang set <cid>
world.lang.usage = @world bruk: @world.lang [show|set <cid>|clear]

# ─── avatar-kommandoer ───────────────────────────────────────────────────────
avatar.look.no-description = Ingenting spesielt her.
avatar.look.no-exits = Det er ingen åpenbare utganger.
avatar.look.exits-label = Utganger:
avatar.look.present-label = Til stede:
avatar.look.things-label = Du ser:
avatar.go.usage = Gå hvor?
avatar.go.no-exit = Ingen utgang
avatar.go.locked = Veien er låst:
avatar.go.denied = Du kan ikke bruke utgang
avatar.look.exit-locked = Den er låst.
avatar.look.exit-open = Den er åpen.
avatar.look.nothing = Du ser ingenting som heter
avatar.inspect.usage = Undersøke hva?
avatar.inspect.not-found = Ingenting å undersøke:
